...
const (
	// DefaultBaseURL is the default URL for the Plain MCP server
	DefaultBaseURL = "https://core-api.uk.plain.com/graphql/v1"
	// DefaultTimeout is the default HTTP timeout
	DefaultTimeout = 30 * time.Second
)

// Client represents a Plain MCP API client
type Client struct {
	baseURL    string
...
// NewClient creates a new Plain MCP client with the given auth token
...
func (c *Client) GetCurrentUser() (*User, error) {
	...
	if err := c.request("POST", "", query, &result); err != nil {
		return nil, err
	}

	if len(result.Errors) > 0 {
		if msg, ok := result.Errors[0]["message"].(string); ok {
			return nil, fmt.Errorf("GraphQL error: %s", msg)
		}
		return nil, fmt.Errorf("GraphQL error: %v", result.Errors[0])
	}

	return &result.Data.MyUser, nil
}

// GetMyThreads fetches threads assigned to the authenticated user
func (c *Client) GetMyThreads(status string, limit, offset int) (*ThreadsResponse, error) {
	...
	query := map[string]interface{}{
		"query": `
			query GetMyThreads($assignedToUser: [ID!], $status: [ThreadStatus!], $first: Int!) {
				threads(
					filters: {
	...
	if status != "" && status != "all" {
		query["variables"].(map[string]interface{})["status"] = []string{status}
	}
	...
	// Convert the response to ThreadsResponse format
	threads := make([]Thread, 0, len(result.Data.Threads.Edges))
	for _, edge := range result.Data.Threads.Edges {
		threads = append(threads, edge.Node)
	}

	return &ThreadsResponse{
		Threads: threads,
		Total:   result.Data.Threads.PageInfo.TotalCount,
	}, nil
}
